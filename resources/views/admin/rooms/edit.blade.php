...
@section('title', 'Edit Kamar - Admin KosKu')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="h4 mb-0"><i class="fas fa-edit me-2"></i>Edit Kamar</h1>
                ...
                <div class="col-md-8">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('nama_kamar') is-invalid @enderror" 
                               id="nama_kamar" name="nama_kamar" value="{{ old('nama_kamar', $room->nama_kamar) }}" required>
                        @error('nama_kamar')
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" class="form-control @error('harga_sewa') is-invalid @enderror" 
                                           id="harga_sewa" name="harga_sewa" value="{{ old('harga_sewa', $room->harga_sewa) }}" required>
                                </div>
                                @error('harga_sewa')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('status') is-invalid @enderror" id="status" name="status" required>
                                    ...
                                    <option value="tidak_tersedia" {{ old('status', $room->status) == 'tidak_tersedia' ? 'selected' : '' }}>Tidak Tersedia</option>
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('latitude') is-invalid @enderror" 
                                       id="latitude" name="latitude" value="{{ old('latitude', $room->latitude) }}">
                                @error('latitude')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control @error('longitude') is-invalid @enderror" 
                                       id="longitude" name="longitude" value="{{ old('longitude', $room->longitude) }}">
                                @error('longitude')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="border rounded p-3" style="max-height: 300px; overflow-y: auto;">
                            @php
                                ...
                                $selectedFasilitas = old('fasilitas', $room->fasilitas ?? []);
                            @endphp
                            @foreach($fasilitasList as $fasilitas)
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="fasilitas[]" 
                                           value="{{ $fasilitas }}" id="fasilitas_{{ $loop->index }}"
...
<script>
    document.querySelector('form').addEventListener('submit', function(e) {
        ...
        submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin me-1"></i>Menyimpan Perubahan...';
    });
</script>
@endsection
@endsection